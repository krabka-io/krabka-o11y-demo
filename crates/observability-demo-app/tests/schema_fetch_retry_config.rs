...
fn environment_schema_fetch_retry_range_is_validated_before_external_io() {
    let output = demo()
        .args(["--role", "produce"])
        .env("KRABKA_DEMO_SCHEMA_FETCH_RETRY_INITIAL_BACKOFF", "91ms")
        .env("KRABKA_DEMO_SCHEMA_FETCH_RETRY_MAX_BACKOFF", "37ms")
        .output()
        .expect("run demo");
...
fn cli_schema_fetch_retry_value_overrides_environment() {
    let output = demo()
        ...
        .env("KRABKA_DEMO_SCHEMA_FETCH_RETRY_INITIAL_BACKOFF", "37ms")
        .env("KRABKA_DEMO_SCHEMA_FETCH_RETRY_MAX_BACKOFF", "91ms")
        .output()
        .expect("run demo");
